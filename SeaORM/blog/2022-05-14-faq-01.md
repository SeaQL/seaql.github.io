...
We faces the dilemma where we either put the object on the stack and waste some space (it takes up 48 bytes no matter `child` is `None` or not) or put the object in a box and waste some cycles.
