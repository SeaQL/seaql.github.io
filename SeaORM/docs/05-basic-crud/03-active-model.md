...
- `Set` - A `Value` that's explicitly set by the application and sent to the database.
    ...
    When editing an existing value, you can use [`set_if_not_equal`](https://docs.rs/sea-orm/2.0.0/sea_orm/entity/enum.ActiveValue.html#method.set_if_not_equals)
    to preserve the `Unchanged` state when the new value is the same as the old one.
    Then you can meaningfully use methods like [`ActiveModelTrait::is_changed`](https://docs.rs/sea-orm/2.0.0/sea_orm/entity/trait.ActiveModelTrait.html#method.is_changed).
...
You can check whether any field in an `ActiveModel` is `Set` with the [`is_changed`](https://docs.rs/sea-orm/2.0.0/sea_orm/entity/prelude/trait.ActiveModelTrait.html#method.is_changed) method.
...
Using [`try_into_model`](https://docs.rs/sea-orm/2.0.0/sea_orm/entity/trait.TryIntoModel.html#tymethod.try_into_model) method you can convert ActiveModel back to Model.
...
If you wish to automatically fill remaining fields with their `Default::default()` values (or fallback to `NotSet` if they don't have one), you can use the [`ActiveModelTrait::default_values()`](https://docs.rs/sea-orm/2.0.0/sea_orm/entity/trait.ActiveModelTrait.html#tymethod.default_values) method.
