...
By implementing the [`EntityTrait`](https://docs.rs/sea-orm/2.0.0/sea_orm/entity/trait.EntityTrait.html), you can perform CRUD operations on the given table.
...
To specify the datatype of each column, the [`ColumnType`](https://docs.rs/sea-orm/2.0.0/sea_orm/entity/enum.ColumnType.html) enum can be used.
...
`ValueType` defines the type of last_insert_id in [`InsertResult`](https://docs.rs/sea-orm/2.0.0/sea_orm/struct.InsertResult.html).
...
An `ActiveModel` has all the attributes of its corresponding `Model` but all attributes are wrapped in an [`ActiveValue`](https://docs.rs/sea-orm/2.0.0/sea_orm/entity/enum.ActiveValue.html).
