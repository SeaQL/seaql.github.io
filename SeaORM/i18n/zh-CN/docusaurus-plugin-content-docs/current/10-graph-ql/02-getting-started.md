...
要开始使用，你只需要一个带有 schema 的可用 SQL 数据库。你可以通过编写 SeaORM 实体在 Rust 中完成所有工作，或使用 GUI 工具（例如 [DataGrip](https://www.jetbrains.com/datagrip/)）设计 schema。
...
你当然可以自由修改项目以满足你的需求。
有趣的部分从 `query_root.rs` 中的 `schema_builder` 函数开始。
