...
We've also added a new `COLUMN` constant to make it more ergonomic.
...
Under the hood, each `Column` value is wrapped in a zero‑sized `COLUMN` constant. This wrapper is generic over `Entity`, so whether a table has 1 column or 100, the compile‑time cost stays roughly the same.
...
Each column type wrapper exposes a set of methods that's relevant for the column's type, so for example `StringColumn::contains` and `ArrayColumn::contains` are now distinct methods that do the right thing!
...
Right now there are `BoolColumn`, `NumericColumn`, `StringColumn`, `BytesColumn`, `JsonColumn`, `DateLikeColumn`, `TimeLikeColumn`, `DateTimeLikeColumn`, `UuidColumn`, `IpNetworkColumn`, and more relevant methods can be added, feel free to make sugguestions.
